/*
 * Description: Ask the user to draw the cell of interest, identify its nucleus, detect RNA puncta within the cell, and compute the distance from each punctum to the nucleus centroid
...
for (i = 0; i < inputFiles.length; i++) {
    if (endsWith(inputFiles[i], ".czi")) {
		...
		// ASK USER TO DRAW CELL OF INTEREST
		setBatchMode(false);
    	// Open cells channel
		...
		// Prompt user to draw cell
		run("Enhance Contrast", "saturated=0.5");
		setTool("polygon"); 					
		Dialog.createNonBlocking("");
		Dialog.addMessage("Please draw the cell of interest and click OK");
		Dialog.show();
		if (getValue("selection.size") == 0) {
			print("ERROR: No cell drawn, image skipped.");
			closeAll();
			continue;
		}
		run("Create Mask");
